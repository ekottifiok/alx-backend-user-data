...
class Auth:
    ...
    def require_auth(self, path: str, excluded_paths: List[str]) -> bool:
        ...
        if path is None or excluded_paths is None:
            return True
        excluded_paths = [item if item[-1] != '/' else item[:-1] for item in excluded_paths]
    ...
    def authorization_header(self, request=None) -> str:
        ...
        Returns:
            str: _description_
        """
        if request is None or not isinstance(request, dict) or 'Authorization' not in request.keys():
            return None
